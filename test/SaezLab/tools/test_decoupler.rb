...
class TestDecoupler < Test::Unit::TestCase
  def _test_decoupler
    ...
    SaezLab._pycall_decoupler do
      mat, net = decoupler_python.get_toy_data()
    end

    acts, acts_normal, pvals = SaezLab.run_decoupler(:gsea, mat, net, min_n: 0, times: 100)
    assert acts.keys.include?("S01")

    acts, acts_normal, pvals = SaezLab.run_decoupler(:viper, mat, net, min_n: 0)
    assert acts.keys.include?("S01")
    assert_nil pvals
  end
